...
# Mission 0 is the shakeout for the whole course: doctor checks everything
# the Foundation module will need, not just what this mission needs.
set -u

MISSION="Mission 0: Reporting for Duty"
...
if [ "${SDC_RUNNING:-0}" -gt 0 ]; then
    ...
    if [ -n "$PORTS_BUSY" ]; then
        fail "Port 2221 free" "Port(s)${PORTS_BUSY} in use — another mission lab or service is listening. Run 'make destroy' in the other mission, or free the port"
    else
        pass "Port 2221 free"
    fi
fi
