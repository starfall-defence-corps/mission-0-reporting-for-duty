...
if [ "$(git rev-list --count "$BASE_REF"..HEAD 2>/dev/null || echo 0)" = "0" ]; then
    die "Something to submit" "Your branch has no changes for ARIA to review. Run 'make test' until all phases pass — on success it writes workspace/reports/mission-0-receipt.txt — then run 'make submit' again"
fi
